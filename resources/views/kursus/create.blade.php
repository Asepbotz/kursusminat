...
<body>
    <div class="container">
        ...
        <form action="{{ route('kursus.store', ['subdomain' => $subdomain]) }}" method="post">
            @csrf
            <div>
                <label for="judul">Judul:</label>
                <input type="text" id="judul" name="judul" placeholder="Judul Kursus" value="{{ old('judul') }}" required>
                @error('judul')
            ...
            <div>
                ...
                <textarea id="deskripsi" name="deskripsi" placeholder="Deskripsi Kursus" rows="5" required>{{ old('deskripsi') }}</textarea>
                @error('deskripsi')
                    <div class="error-message">{{ $message }}</div>
                @enderror
            </div>
